...
class ItechOfficailRepresentative extends \CBitrixComponent
{
    public $setSections = [];
...
//        $this->arResult['SECTIONS'] = $elements;
...
//                ];
//                $sections[$arElement['FIELDS']['IBLOCK_SECTION_ID']]['ITEMS'][] = $arElement;
//            }
//            foreach ($sections as $key => $section){
//                if (!$section['ITEMS']){
//                    unset($sections[$key]);
...
//            return $sections;
    ...
    private function getCityId($city)
    {
        if (CModule::IncludeModule('iblock')) {
            $arSelect = [];
            $arFilter = ['IBLOCK_ID' => IBLOCK_ID_CIRIES, 'NAME' => $city['NAME'], 'ACTIVE' => 'Y'];
