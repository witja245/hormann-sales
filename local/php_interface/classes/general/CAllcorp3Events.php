...
class CAllcorp3Events{
	...
	static function onAfterResultAddHandler($WEB_FORM_ID, $RESULT_ID){
		...
		if(Option::get(self::MODULE_ID, 'AUTOMATE_SEND_ACLOUD', 'Y') == 'Y')
			\Aspro\Functions\CAsproAllcorp3::sendLeadCrmFromForm($WEB_FORM_ID, $RESULT_ID, 'ACLOUD', SITE_ID, false, false);
	...
	static function OnAfterIBlockElementAddHandler($arFields){
		if ($arFields['IBLOCK_ID'] != ($iblockID = \CAllcorp3Cache::$arIBlocks[SITE_ID]["aspro_allcorp3_content"]["aspro_allcorp3_reviews"][0])) {
			$PROP = [];
