...
if(!class_exists('CAllcorp3Cache')){
	class CAllcorp3Cache {
		...
		public static function ClearTagIBlockSectionBeforeDelete($ID){
			global $CACHE_MANAGER;
			if($ID > 0){
				if($IBLOCK_ID = self::CIBlockSection_GetList(array("CACHE" => array("MULTI" => "N")), array("ID" => $ID), false, array("IBLOCK_ID"), true)){
					$CACHE_MANAGER->ClearByTag(self::GetIBlockCacheTag($IBLOCK_ID));
				}
			}
		}
	}
