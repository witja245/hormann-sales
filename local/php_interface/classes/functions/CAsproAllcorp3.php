...
class CAsproAllcorp3
{
    ...
    public static function showSchemaAvailabilityMeta($status = "")
    {
        if (!empty($status)) {
            switch ($status) {
                ...
                case "order":
                    return '<link itemprop="availability" href="http://schema.org/PreOrder">';
                case "pending":
                    return '<link itemprop="availability" href="http://schema.org/PreSale">';
            }
        }
    }
    ...
    public static function showHeaderBlock($options)
    {
        ...
        <div class="<?= $options['WRAPPER'] ? $options['WRAPPER'] : '' ?> <?= $options['VISIBLE'] ? '' : 'hidden' ?>"
             data-ajax-load-block="<?= $options['PARAM_NAME'] ?>">
            <? if ($options['VISIBLE']): ?>
                ...
                switch ($options['BLOCK_TYPE']) {
                        ...
                        <div class="<?= $options['NO_ICON'] ? '' : 'icon-block--with_icon' ?> <?= $options['ONLY_ICON'] ? 'icon-block--only_icon' : '' ?>">
                            <div class="phones">
                                ...
                                <div class="phones__phones-wrapper">
                                    ...
                                    <? \CAllcorp3::ShowHeaderPhones('phones__inner--big', $svg, $options['ONLY_ICON'], $options['DROPDOWN_TOP']); ?>
                                </div>

                                <? if ($options['CALLBACK']): ?>
                                    <div class="phones__callback light-opacity-hover animate-load colored banner-light-text menu-light-text <?= $options['CALLBACK_CLASS'] ? $options['CALLBACK_CLASS'] : '' ?>"
                                         data-event="jqm"
                                         data-param-id="<?= \CAllcorp3::getFormID("aspro_allcorp3_callback"); ?>"
                                         data-name="callback">
                                        <?= $options['MESSAGE'] ?>
                                    </div>
                                <? endif; ?>
                    ...
                    case 'EYED':
                        ...
                        ?>
                        <div class="header-eyed eyed-toggle <?= ($bEyedActive ? 'eyed-toggle--off' : 'eyed-toggle--on') ?> banner-light-icon-fill fill-theme-hover color-theme-hover menu-light-icon-fill light-opacity-hover"
                             title="<?= $titleEyed ?>">
    ...
    public static function showFooterBlock($options)
    {
        ...
        <div class="<?= $options['WRAPPER'] ? $options['WRAPPER'] : '' ?> <?= $options['VISIBLE'] ? '' : 'hidden' ?>"
             data-ajax-load-block="<?= $options['PARAM_NAME'] ?>"
             data-ajax-check-visible="<?= ($options['PARAM_CHECK_VISIBLE'] ? $options['PARAM_CHECK_VISIBLE'] : ''); ?>">
                ...
                <? if ($options['VISIBLE']): ?>
                    ...
                    switch ($options['BLOCK_TYPE']) {
                        case 'SUBSCRIBE':
                            ...
                            <? if ($options['COMPACT']): ?>
                                <div class="stroke-theme-parent-all color-theme-parent-all">
                                    <div class="icon-block icon-block--with_icon" data-event="jqm"
                        ...
                        case 'EYED':
                            ...
                            $titleEyed = $bEyedActive ? Loc::getMessage("EA_T_NORMAL_VERSION_SHORT") : Loc::getMessage("EA_T_EYED_VERSION");
                            ?>
                            <div class="footer__eyed eyed-toggle <?= ($bEyedActive ? 'eyed-toggle--off' : 'eyed-toggle--on') ?> font_13 color_999 pointer"
                                 title="<?= $titleEyed ?>">
    ...
    <?endif;
        ...
        <? if (!$bOrderViewBasket || $bOrderViewBasket === 'false'): ?>
            <span class="buy_block btn-actions__inner">
							<span class="buttons">
								<span class="btn btn-default <?= $arConfig['BTN_CLASS'] ?> <?= $arConfig['BTN_CLASS_MORE'] ?> animate-load"
                                      data-event="jqm"
                                      data-param-id="<?= \CAllcorp3::getFormID($arConfig['ORDER_FORM_ID']); ?>"
                                      data-autoload-product="<?= \CAllcorp3::formatJsName($arItem["NAME"]); ?>"
                                      data-autoload-service="<?= \CAllcorp3::formatJsName($arItem["NAME"]); ?>"
                                      data-autoload-project="<?= \CAllcorp3::formatJsName($arItem["NAME"]); ?>"
                                      data-autoload-news="<?= \CAllcorp3::formatJsName($arItem["NAME"]); ?>"
                                      data-autoload-sale="<?= \CAllcorp3::formatJsName($arItem["NAME"]); ?>"
                                      data-name="order_product_<?= $arItem['ID']; ?>">
									<?= $sOrderText; ?>
								</span>
							</span>
                                ...
                                <? if (
                                    ...
                                    <div class="info-buttons flexbox flexbox--direction-row">
                                        ...
                                        <? if ($arConfig['DISPLAY_COMPARE']): ?>
                                            ...
                                            <div class="info-buttons__item info-buttons__item--question info-buttons__item--compare <?= (!$arConfig['QUESTION_BTN'] ? 'flex-1' : ''); ?>">
												<a
                                                        href="javascript:void(0)"
                                                        rel="nofollow"
        ...
        <? else: ?>
            <div class="buy_block btn-actions__inner">
                ...
                <? endif; ?>
                <div class="buttons">
								<span class="btn btn-default <?= $arConfig['BTN_CLASS'] ?> <?= $arConfig['BTN_CLASS_MORE'] ?> to_cart animate-load"
                                      data-quantity="1">
									<span>
										<?= $sToCartText ?>
									</span>
    ...
    <? else: ?>
        <? if (!$bDetail): ?>
            ...
            <a href="<?= $arItem['DETAIL_PAGE_URL'] ?>"
               class="btn btn-default btn-actions__inner btn-wide <?= $arConfig['BTN_CLASS'] ?> <?= $arConfig['BTN_CLASS_MORE'] ?> js-replace-more">
                <?= $sMoreText; ?>
            </a>
        <? endif; ?>
    <? endif; ?>
    ...
    public static function getItemMapHtml($arOptions)
    {
        ...
        $arItem['NAME_FORM'] = str_replace(chr(38), "", $arItem['NAME_FORM']);
        $arItem['NAME_FORM'] = str_replace("quot;", "", $arItem['NAME_FORM']);
        ...
        if ($arItem['METRO'] || $arItem['SCHEDULE'] || $arItem['EMAIL'] || $arItem['PHONE']) {
            ...
            if ($arItem['REGION']) {
                $html .= '<div class="region-info" data-region="' . $arItem['REGION'] . '"></div>';
            }
            $html .= '</div>';
        }
        ...
        if ($bShowQuestionBtn) {
            ...
            $html .= '<span class="btn btn-default  btn-transparent-border animate-load ' . $btnClass . '" data-region="' . $arItem['REGION'] . '" data-dealer="' . $arItem['NAME_FORM'] . '" data-event="jqm"  data-param-id="' . \CAllcorp3::getFormID("question_dealer_detail_s1") . '" data-name="question">' . GetMessage('SEND_MESSAGE_BUTTON') . '</span>';
            $html .= '</div>';
        }
        // $html .= '</div>';
    ...
    public static function showDiscountCounter($arOptions = [])
    {
        ...
        <div class="countdown countdown-<?= $arConfig['TYPE'] ?><?= ($arConfig['ICONS'] ? ' countdown--icons' : ''); ?><?= ($arConfig['IS_COMPACT'] ? ' compact' : ''); ?>">
            ...
            <div class="countdown__inner bordered rounded-3">
                ...
                <? endif; ?>
                <span class="countdown__items"><span class="countdown__item">0</span><span
                            class="countdown__item">0</span><span class="countdown__item">0</span><span
                            class="countdown__item">0</span></span>
            </div>
    ...
    public static function showTitleInLeftBlock($arOptions = [])
    {
        ...
        <div class="sticky-block <?= $arConfig['STICKY_CLASS'] ?>">
            ...
            <? if ($bShowTitle): ?>
                <h2 class="index-block__title switcher-title<?= ($bShowContent ? ' line' : '') ?>"><?= $arConfig['PARAMS']['TITLE'] ?></h2>
            <? endif; ?>
            ...
            <? if ($bShowTitleLink): ?>
                <a href="<?= preg_replace('/(?<!:)[\/]{2,}/', '/', SITE_DIR . $arConfig['PARAMS']['RIGHT_LINK']) ?>"
                   class="btn btn-transparent-border btn-lg index-block__btn"><?= $arConfig['PARAMS']['RIGHT_TITLE']; ?></a>
            <? endif; ?>
        </div>
    ...
    public static function showTitleBlock($arOptions = [])
    {
        ...
        <a class="index-block__link dark_link stroke-theme-hover right_link_block"
           href="<?= $arConfig['PARAMS']['RIGHT_LINK'] ?>" target="_blank" rel="nofollow">
        <? else: ?>
        <a class="index-block__link dark_link stroke-theme-hover right_link_block"
           href="<?= preg_replace('/(?<!:)[\/]{2,}/', '/', SITE_DIR . $arConfig['PARAMS']['RIGHT_LINK']) ?>"
           title="<?= Loc::getMessage('REDIRECT_SECTION') ?>">
            <?endif;
            ?>
            ...
            <? if ($arItem["PROPERTIES"][$prop]['VALUE_XML_ID'] ||
                ...
                <div class="sticker sticker--upper">
                    ...
                    <? foreach ($arItem["PROPERTIES"][$prop]['VALUE_XML_ID'] as $key => $class): ?>
                        <div>
                            <div class="sticker__item sticker_item--<?= strtolower($class); ?> font_9"><?= $arItem['PROPERTIES']['HIT']['VALUE'][$key] ?></div>
                        </div>
                    <? endforeach; ?>

                    <? if ($arItem["PROPERTIES"][$saleSticker]["VALUE"]): ?>
                        <div>
                            <div class="sticker__item sticker_item--sale-text font_9"><?= $arItem["PROPERTIES"][$saleSticker]["VALUE"]; ?></div>
                        </div>
                    <? endif; ?>
                </div>
                <? if ($arConfig['WRAPPER']): ?>
            ...
            <div class="side-icons js-replace-icons <?= $arConfig['DOP_CLASS']; ?>">
                <? if ($arParams['USE_FAST_VIEW_PAGE_DETAIL'] != 'NO'): ?>
                    ...
                    <div class="side-icons__item side-icons__item--fast-view bordered rounded-4">
                        <a href="javascript:void(0)" rel="nofollow" data-event="jqm" title="<?= $sFastOrderText ?>"
                           data-name="fast_view" data-param-form_id="fast_view"
                           data-param-iblock_id="<?= $arItem['IBLOCK_ID'] ?>" data-param-id="<?= $arItem['ID'] ?>"
                ...
                <? if ($arParams['SHOW_ONE_CLICK_BUY'] != 'N' && $bOrderButton && $bOrderViewBasket): ?>
                    ...
                    <div class="side-icons__item side-icons__item--ocb bordered rounded-4">
                        <a href="javascript:void(0)" rel="nofollow" class="ocb"
                           data-param-id="<?= \CAllcorp3::getFormID("aspro_allcorp3_quick_buy"); ?>" data-name="ocb"
                           data-event="jqm" title="<?= Loc::getMessage('ONE_CLICK_BUY') ?>"
                           data-autoload-product_name="<?= \CAllcorp3::formatJsName($arItem['NAME']) ?>"
                           data-autoload-product_price="<?= \CAllcorp3::formatJsName($price) ?>">
                ...
                <? if ($arParams['DISPLAY_COMPARE']): ?>
                    <div class="side-icons__item side-icons__item--compare side-icons__item--fill bordered rounded-4">
                        ...
                        <a href="javascript:void(0)" rel="nofollow" class="js-item-action" data-action="compare"
                           data-id="<?= $arTransferData['ID']; ?>"
                           data-item_compare='<?= str_replace('\'', '"', \CUtil::PhpToJsObject($arTransferData)) ?>'
                           title="<?= Loc::getMessage('COMPARE_ITEM') ?>"
                           data-title="<?= Loc::getMessage('COMPARE_ITEM') ?>"
    ...
    public static function showImg($arOptions = [])
    {
        ...
        <? if (!empty($arItem["PREVIEW_PICTURE"])): ?>
            ...
            ?>
            <img class="img-responsive <?= $dopClassImg; ?>" src="<?= $src; ?>"
                 data-big="<?= $bigSrc ?>" <?= $jsImgSrc; ?> alt="<?= $a_alt; ?>" title="<?= $a_title; ?>"/>
        <? elseif (!empty($arItem["DETAIL_PICTURE"])): ?>
            ...
            <? endif; ?>
            <img class="img-responsive <?= $dopClassImg; ?>" src="<?= $img["src"] ?>" <?= $jsImgSrc; ?>
                 alt="<?= $a_alt; ?>" title="<?= $a_title; ?>"/>
        <? else: ?>
            <img class="img-responsive <?= $dopClassImg; ?>"
                 src="<?= SITE_TEMPLATE_PATH . '/images/svg/noimage_product.svg'; ?>" alt="<?= $a_alt; ?>"
                 title="<?= $a_title; ?>"/>
        <? endif; ?>
        <? if ($arConfig['WRAP_LINK']): ?>
            </a>
    ...
    public static function showSectionGallery($arOptions = [])
    {
        ...
        if ($arItem):?>
            ...
            <? if ($arItem[$key]): ?>
                ...
                <span class="section-gallery-wrapper js-replace-gallery flexbox">
								<? foreach ($arItem[$key] as $i => $arGalleryItem): ?>
                                    ...
                                    <span class="section-gallery-wrapper__item<?= (!$i ? ' active' : ''); ?>">
										...
										<img class="img-responsive <?= $dopClassImg ?>"
                                             src="<?= $arGalleryItem["SRC"]; ?>" <?= $jsImgSrc; ?> data-big="<?= $arGalleryItem["SRC"] ?>"
                                             alt="<?= $arGalleryItem["ALT"] ?? $arItem['NAME']; ?>"
                                             title="<?= $arGalleryItem["TITLE"] ?? $arItem['NAME']; ?>"/>
                                        <meta itemprop="image" content="<?= $arGalleryItem["SRC"]; ?>">
									</span>
                                <? endforeach; ?>
							</span>
                <? if ($arConfig['WRAP_LINK']): ?>
                ...
                <? endif; ?>
            <? else: ?>
                <a href="<?= $arItem["DETAIL_PAGE_URL"] ?>" class="image-list__link"><img
                            class="img-responsive <?= $dopClassImg ?>"
                            src="<?= SITE_TEMPLATE_PATH . '/images/svg/noimage_product.svg'; ?>"
                            alt="<?= $arItem["NAME"] ?>" title="<?= $arItem["NAME"] ?>"/></a>
            <? endif; ?>
            <? $html = ob_get_contents();
            ob_end_clean();
                                        ...
                                        <? if (is_array($arImg)): ?>
                                            ...
                                            <? else: ?>
                                                <img class="bottom-icons-panel__content-picture lazyload"
                                                     src="<?= $arImg["src"] ?>" data-src="<?= $arImg["src"] ?>"
    ...
    public static function showRSSIcon(array $arOptions)
    {
        ...
        ob_start();
        ?>
        <div class="rss fill-theme-hover"><a href="<?= $url ?>" title="RSS"
                                             target="_blank"><?= \CAllcorp3::showIconSvg('print', SITE_TEMPLATE_PATH . '/images/svg/rss.svg', '', 'colored_theme_hover_bg-el-svg') ?></a>
        </div>
        <?
        $html = ob_get_contents();
        ob_end_clean();
    ...
    public static function showBackUrl(array $arOptions)
    {
        ...
        ob_start();
        ?>
        <a class="back-url font_14 dark_link stroke-theme-parent-all colored_theme_hover_bg-block animate-arrow-hover color-theme-parent-all"
           href="<?= $url ?>">
    ...
    public static function checkProperty($nameDownloadFile, $urlDownloadFile)
    {
        if ($urlDownloadFile) {
            ...
            ob_start();
            ?>
            <div class="download fill-theme-hover"><a download href="<?= $urlDownloadFile ?>"
                                                      title="<?= $nameDownloadFile ?>"><?= \CAllcorp3::showIconSvg('download', SITE_TEMPLATE_PATH . '/images/svg/file_download.svg', '', 'colored_theme_hover_bg-el-svg') ?>
                    <span class="font_13 title"><?= $nameDownloadFile; ?></span></a></div>
            <?
            $html = ob_get_contents();
            ob_end_clean();
            return $html;
